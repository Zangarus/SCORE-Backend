...
export class FirstScoreAlgorithm implements IScoreAlgorithm {
    ...
    proceed(data: Entry, oldScore: Score): Score {
        ...
        oldScore = this.calc_emission(oldScore);
        return oldScore;
    }
    private calc_emission(score: Score): Score {
        const emCar = score.distanceCar * AC.CO2_A;
        const emFartrain = score.distanceFartrain * AC.CO2_FB;
        const emNeartrain = score.distanceNeartrain * AC.CO2_NB;
        const emFoot = score.distanceFoot * AC.CO2_FU;
        const emBike = score.distanceBike * AC.CO2_FR;
        const emEBike = score.distanceEbike * AC.CO2_EF;
        const emECar = score.distanceEcar * AC.CO2_EA;
        const emNearPlane = score.distancePlane * AC.CO2_FZ;
        
        score.emScore = emCar + emFartrain + emNeartrain + emFoot + emBike + emEBike + emECar + emNearPlane;
        return score;
    }
}